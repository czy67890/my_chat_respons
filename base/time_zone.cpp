...
time_t TimeZone::from_local_time(const struct tm& localTm) const
{
  ...
  if (localTm.tm_isdst)
  {
    struct tm tryTm = to_local_time(seconds - local->gmtOffset);
    if (!tryTm.tm_isdst
...
struct tm TimeZone::to_utc_time(time_t secondsSinceEpoch, bool yday)
{
  ...
  Date date(days + Date::k_julian_day_of_1970_01_01);
...
time_t TimeZone::from_utc_time(int year, int month, int day,
                             int hour, int minute, int seconds)
{
  Date date(year, month, day);
  int secondsInDay = hour * 3600 + minute * 60 + seconds;
  time_t days = date.julian_day_number() - Date::k_julian_day_of_1970_01_01;
